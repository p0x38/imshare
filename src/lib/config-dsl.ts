...
const SIZE_UNITS: Record<string, number> = { B: 1, KB: 1_000, MB: 1_000_000, GB: 1_000_000_000, KiB: 1024, MiB: 1024 ** 2, GiB: 1024 ** 3 };
const DURATION_UNITS: Record<string, number> = { ms: 1, s: 1_000, m: 60_000, h: 3_600_000, d: 86_400_000 };

export function parseConfigDsl(content: string): unknown { return new Parser(tokenize(content)).parse(); }
...
function tokenize(source: string): Token[] {
    ...
    while (index < source.length) {
        ...
        if ("{}[]=,.".includes(char)) { tokens.push({ type: "symbol", value: char, line, column }); advance(); continue; }
...
class Parser {
    ...
    private parseValue(): unknown {
        ...
        if (token.type === "string") { this.advance(); return JSON.parse(token.value) as string; }
        if (token.type === "boolean") { this.advance(); return token.value === "true"; }
        if (token.type === "number") { this.advance(); return parseNumber(token); }
        if (token.type === "identifier" && token.value === "null") { this.advance(); return null; }
    ...
    private expectSymbol(value: string): Token { const token = this.current(); if (token.type !== "symbol" || token.value !== value) this.fail(`Expected '${value}'`); this.index++; return token; }
    private skipNewlines(): void { while (this.at("newline")) this.advance(); }
    private fail(message: string): never { const token = this.current(); throw new ConfigDslError(message, token.line, token.column); }
}
