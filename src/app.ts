...
export async function buildApp() {
  const config = await loadConfig(); const app = Fastify({ logger, disableRequestLogging: false }); const rootDir = process.cwd();
  const publicDir = path.join(rootDir, "public"); const uploadDir = path.resolve(rootDir, config.storage.uploadDirectory); await mkdir(uploadDir, { recursive: true });
  await app.register(cookie); await app.register(multipart, { limits: { fileSize: config.storage.maxFileSize, files: 20 } }); await app.register(authRoutes); await app.register(apiRoutes);
  ...
  app.get("/", sendPage("index.html")); app.get("/posts/", sendPage("posts/index.html")); app.get("/posts/:postId", sendPage("posts/view.html"));
  ...
  app.get("/search/", sendPage("search/index.html")); app.get("/account/", sendPage("account/index.html")); app.get("/account/login/", sendPage("account/login/index.html")); app.get("/account/register/", sendPage("account/register/index.html")); app.get("/notifications/", sendPage("account/notifications.html"));
