...
export async function buildApp() {
  ...
  app.addHook("onRequest", async (request, reply) => {
    ...
    if (request.method === "POST" && request.url.split("?", 1)[0] === "/v1/uploads") {
      ...
      if (!result.allowed) return reply.code(429).header("Retry-After", String(result.retryAfter)).send({ error: { code: "UPLOAD_RATE_LIMITED", message: "Upload limit exceeded. Try again later." } });
      return;
    }
    if (request.method === "GET" && !request.url.startsWith("/v1/health")) {
      ...
      if (!result.allowed) return reply.code(429).header("Retry-After", String(result.retryAfter)).send({ error: { code: "RATE_LIMITED", message: "Too many requests. Try again later." } });
    }
  ...
  app.get("/account/logout/", async (request, reply) => {
    const response = await fetch(`${request.protocol}://${request.hostname}/v1/auth/sign-out`, { method: "POST", headers: { cookie: request.headers.cookie ?? "" } });
  ...
  app.setNotFoundHandler(async (request, reply) => {
    if ((request.headers.accept ?? "").includes("text/html")) return reply.code(404).type("text/html").send(await renderTemplate("error.html", { status: 404, title: "Page not found", message: "The page you requested does not exist." }));
    return reply.code(404).send({ error: { code: "NOT_FOUND", message: "The requested page or resource was not found." } });
  });

  app.setErrorHandler(async (error, request, reply) => {
    request.log.error(error); if (reply.sent) return;
    const errorObject = typeof error === "object" && error !== null ? error : undefined;
    const statusCode = errorObject && "statusCode" in errorObject && typeof errorObject.statusCode === "number" ? errorObject.statusCode : 500;
    const status = statusCode >= 400 ? statusCode : 500;
    const message = error instanceof Error ? error.message : errorObject && "message" in errorObject && typeof errorObject.message === "string" ? errorObject.message : "Something went wrong while processing your request.";
    const code = errorObject && "code" in errorObject && typeof errorObject.code === "string" ? errorObject.code : "INTERNAL_ERROR";
    const publicMessage = status < 500 ? message : "Something went wrong while processing your request.";
    if ((request.headers.accept ?? "").includes("text/html")) return reply.code(status).type("text/html").send(await renderTemplate("error.html", { status, title: status === 404 ? "Page not found" : "Something went wrong", message: publicMessage }));
