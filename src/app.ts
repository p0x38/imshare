...
export async function buildApp() {
  ...
  app.setNotFoundHandler(async (request, reply) => {
    if ((request.headers.accept ?? "").includes("text/html")) {
      return reply.code(404).type("text/html").send(
        await renderTemplate("error.html", {
          status: 404,
  ...
  app.setErrorHandler(async (error, request, reply) => {
    ...
    const statusCode = errorObject && "statusCode" in errorObject && typeof errorObject.statusCode === "number"
    ...
    const code = errorObject && "code" in errorObject && typeof errorObject.code === "string"
      ? errorObject.code
      : "INTERNAL_ERROR";
