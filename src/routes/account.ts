...
export const accountRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.patch("/v1/me/preferences", async (request, reply) => {
        ...
        const booleanKeys = [
            "isPublic", "showEmail", "showPosts", "showProfile", "showHandle",
        ...
        const updated = await prisma.user.update({
            ...
            select: {
                isPublic: true, showEmail: true, showPosts: true, showProfile: true, showHandle: true,
                showFollowers: true, showFollowings: true, allowSearchEngineIndex: true,
    ...
    fastify.get(
        "/v1/me/sessions",
        { schema: openapi({ tags: "Users", summary: "List current user's sessions", description: "Returns authenticated sessions for the current user, including whether each session is the current session. Supports page and limit query parameters." }) },
        async (request, reply) => {
            const user = await requireUser(request, reply);
            if (!user) return;
    ...
    fastify.delete(
        ...
        { schema: openapi({ tags: "Users", summary: "Revoke a session", description: "Revokes one session owned by the authenticated user.", responseExamples: { "200": { data: { revoked: true } }, "404": { error: { code: "SESSION_NOT_FOUND", message: "Session not found." } } } }) },
        async (request, reply) => {
            const user = await requireUser(request, reply);
            if (!user) return;
