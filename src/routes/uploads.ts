...
function uploadView(upload: any) { return { ...upload, url: `/v1/posts/image/${encodeURIComponent(upload.id)}` }; }

export const uploadRoutes: FastifyPluginAsync = async (fastify) => {
  const config = await loadConfig(); const uploadDir = path.resolve(process.cwd(), config.storage.uploadDirectory); await mkdir(uploadDir, { recursive: true });
  fastify.post("/v1/uploads", async (request, reply) => {
    const user = await requireUser(request, reply); if (!user) return;
    const multiple = (request.query as Record<string, unknown>).multiple === "true"; const parts = request.parts({ limits: { files: multiple ? 20 : 1, fileSize: config.storage.maxFileSize, fields: 4 } }); const uploads: any[] = [];
    try { for await (const part of parts) {
      if (part.type !== "file") continue; const uploadId = randomUUID(); const ext = path.extname(part.filename).toLowerCase(); const mime = IMAGE_TYPES.get(ext);
      if (!mime || part.mimetype !== mime) { part.file.resume(); broadcastUploadStatus(user.id, { uploadId, status: "failed", error: "Unsupported image type." }); return reply.code(400).send({ error: { code: "INVALID_FILE", message: "Unsupported image type." } }); }
      broadcastUploadStatus(user.id, { uploadId, status: "uploading", progress: 0 }); const temporaryPath = path.join(uploadDir, `.upload-${uploadId}${ext}`);
      try { await pipeline(part.file, createWriteStream(temporaryPath, { flags: "wx" })); } catch (error) { await unlink(temporaryPath).catch(() => undefined); const cancelled = (error as NodeJS.ErrnoException).name === "AbortError" || request.raw.destroyed; broadcastUploadStatus(user.id, { uploadId, status: cancelled ? "cancelled" : "failed", error: cancelled ? undefined : "Upload failed." }); if (cancelled) return reply.code(499).send({ error: { code: "UPLOAD_CANCELLED", message: "Upload cancelled." } }); throw error; }
      if (part.file.truncated) { await unlink(temporaryPath).catch(() => undefined); broadcastUploadStatus(user.id, { uploadId, status: "failed", error: "Maximum file size exceeded." }); return reply.code(413).send({ error: { code: "FILE_TOO_LARGE", message: "Maximum file size exceeded." } }); }
      broadcastUploadStatus(user.id, { uploadId, status: "processing", progress: 100 }); const content = await readFile(temporaryPath);
      if (!hasImageSignature(content.subarray(0, 32), ext)) { await unlink(temporaryPath).catch(() => undefined); broadcastUploadStatus(user.id, { uploadId, status: "failed", error: "Invalid image content." }); return reply.code(400).send({ error: { code: "INVALID_FILE", message: "File content does not match its image type." } }); }
      ...
      try { const upload = await prisma.upload.create({ data: { id: uploadId, filename, originalName: part.filename, mimeType: part.mimetype, size: content.byteLength, contentHash, userId: user.id } }); uploads.push(upload); broadcastUploadStatus(user.id, { uploadId, status: "ready", progress: 100, url: uploadView(upload).url }); }
  ...
  fastify.delete("/v1/uploads/:uploadId", async (request, reply) => { const user = await requireUser(request, reply); if (!user) return; const { uploadId } = request.params as { uploadId: string }; const upload = await prisma.upload.findUnique({ where: { id: uploadId } }); if (!upload) return reply.code(404).send({ error: { code: "UPLOAD_NOT_FOUND", message: "Upload not found." } }); if (upload.userId !== user.id) return reply.code(403).send({ error: { code: "FORBIDDEN", message: "You do not own this upload." } }); if (upload.postId) return reply.code(409).send({ error: { code: "UPLOAD_IN_USE", message: "Upload is attached to a post." } }); await prisma.upload.delete({ where: { id: uploadId } }); const stillReferenced = await prisma.upload.count({ where: { filename: upload.filename } }); if (!stillReferenced) await unlink(path.join(uploadDir, upload.filename)).catch(() => undefined); return reply.code(204).send(); });
};
