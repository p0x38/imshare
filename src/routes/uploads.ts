...
export const uploadRoutes: FastifyPluginAsync = async (fastify) => {
  const config = await loadConfig(); const uploadDir = path.resolve(process.cwd(), config.storage.uploadDirectory); const cacheDir = path.join(uploadDir, ".cache"); await mkdir(uploadDir, { recursive: true });
