...
export const pushRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.delete("/v1/me/push-subscriptions/:subscriptionId", {
        schema: openapi({
            tags: "Notifications",
            summary: "Remove a Web Push subscription",
            description: "Removes one push subscription belonging to the authenticated user.",
            operationId: "removePushSubscription",
            parameters: [parameter.path("subscriptionId", { type: "string" })],
            security: [{ cookieAuth: [] }],
