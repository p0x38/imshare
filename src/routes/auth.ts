...
export const authRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get("/v1/config", {
        schema: openapi({
            tags: "Authentication",
            summary: "Get public configuration",
            description: "Returns the subset of imshare configuration intended for public clients.",
    ...
    fastify.get("/v1/registration-token", {
        schema: openapi({
            tags: "Authentication",
            summary: "Get a registration token",
            description: "Returns the current registration access token when registration is configured to require one.",
    ...
    fastify.route({
        ...
        schema: openapi({
            ...
            description: "Passes supported Better Auth operations through the imshare authentication adapter. The concrete operation and request schema depend on the authentication sub-path.",
        ...
        handler: async (request, reply) => {
            try {
                ...
                return reply.code(500).send({ error: { code: "AUTH_FAILURE", message: "Authentication request failed." } });
            }
        },
    });
};
