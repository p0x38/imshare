...
export const permalinkRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get("/v1/posts/permalink/:key", async (request, reply) => {
        ...
        const post = await prisma.post.findUnique({ where: { permalinkKey: decodeURIComponent(key) }, include: postInclude });
        ...
        if (!owner && !publicVisible && !unlistedVisible) return reply.code(404).send({ error: { code: "POST_NOT_FOUND", message: "Post not found." } });
