...
export const profileLinkRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/v1/me/links", async (request, reply) => {
    ...
    if (!label || label.length > 100 || !url) {
      return reply.code(400).send({ error: { code: "INVALID_PROFILE_LINK", message: "A label and an HTTP(S) URL are required." } });
    }
    ...
    return reply.code(201).send(ok(await prisma.profileLink.create({ data: {
      userId: user.id,
      label,
      url,
  ...
  fastify.patch("/v1/me/links/:linkId", async (request, reply) => {
    ...
    return ok(await prisma.profileLink.update({ where: { id: linkId }, data: {
      label,
      url,
      ...(body.position !== undefined ? { position: Math.max(0, Math.min(999, Math.trunc(body.position))) } : {}),
    } }));
  });
