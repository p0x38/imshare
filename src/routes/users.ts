...
const RESERVED_HANDLES = new Set(["admin", "api", "account", "dashboard", "login", "logout", "signup", "register", "users", "posts", "tags", "categories", "search", "about", "faq", "github", "privacy", "terms"]);

const publicUserSelect = {
    id: true, name: true, handle: true, image: true, bio: true, websiteUrl: true, githubUrl: true,
    avatarMode: true, avatarValue: true, profileBannerUrl: true, accentColor: true, updatedAt: true, createdAt: true,
...
async function publicUser(userId: string) {
    const user = await prisma.user.findUnique({ where: { id: userId }, select: { ...publicUserSelect, profileLinks: { orderBy: [{ position: "asc" }, { createdAt: "asc" }] } } });
...
async function publicUserByHandle(handle: string) {
    ...
    const user = await prisma.user.findUnique({ where: { handle: normalized }, select: { ...publicUserSelect, profileLinks: { orderBy: [{ position: "asc" }, { createdAt: "asc" }] } } });
    if (!user) return undefined;
    return { ...user, avatarUrl: avatarUrl(user.id, user.updatedAt) };
}
...
export const userRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get("/v1/me/posts", async (request, reply) => {
        const user = await requireUser(request, reply); if (!user) return;
        const q = request.query as Record<string, unknown>; const p = parsePagination(q); const where = { userId: user.id };
    ...
    fastify.get("/v1/users", async (request) => {
        const q = request.query as Record<string, unknown>; const p = parsePagination(q); const search = typeof q.search === "string" ? q.search : undefined;
        const where = search ? { OR: [{ name: { contains: search } }, { email: { contains: search } }, { handle: { contains: normalizeHandle(search) ?? search.toLowerCase() } }] } : {};
    ...
    fastify.post("/v1/users", { schema: userCreateSchema }, async (request, reply) => {
        ...
        const handle = normalizeHandle(body.handle); const handleError = validateHandle(handle);
        if (handleError) return reply.code(400).send({ error: { code: "INVALID_HANDLE", message: handleError } });
        try {
            return reply.code(201).send(ok(await prisma.user.create({ data: { id: randomUUID(), name: body.name.trim(), handle, email: body.email.trim(), image: body.image, bio: body.bio?.trim() || undefined, websiteUrl: body.websiteUrl?.trim() || undefined, githubUrl: body.githubUrl?.trim() || undefined } })));
        } catch (error) {
    ...
    fastify.get("/v1/users/@:handle", async (request, reply) => {
        const { handle } = request.params as { handle: string }; const user = await publicUserByHandle(handle);
    ...
    fastify.get("/v1/users/:userId", async (request, reply) => {
        const { userId } = request.params as { userId: string }; const user = await publicUser(userId);
        if (!user) return reply.code(404).send({ error: { code: "USER_NOT_FOUND", message: "User not found." } });
        return ok(user);
    });
    fastify.patch("/v1/users/:userId", { schema: userUpdateSchema }, async (request, reply) => {
        ...
        const body = request.body as { name?: string; handle?: string | null; image?: string | null; bio?: string | null; websiteUrl?: string | null; githubUrl?: string | null; avatarMode?: string; avatarValue?: string | null; profileBannerUrl?: string | null; accentColor?: string | null };
        const handle = body.handle === undefined ? undefined : normalizeHandle(body.handle); const handleError = validateHandle(handle ?? null);
        if (handleError) return reply.code(400).send({ error: { code: "INVALID_HANDLE", message: handleError } });
        if (body.avatarMode !== undefined && !["default", "initials", "identicon", "gravatar", "custom"].includes(body.avatarMode)) return reply.code(400).send({ error: { code: "INVALID_AVATAR_MODE", message: "Unsupported avatar mode." } });
        try {
            const updated = await prisma.user.update({ where: { id: userId }, data: {
                ...(body.name !== undefined ? { name: body.name.trim() } : {}), ...(body.handle !== undefined ? { handle } : {}),
                ...(body.image !== undefined ? { image: body.image } : {}), ...(body.bio !== undefined ? { bio: body.bio?.trim() || null } : {}),
                ...(body.websiteUrl !== undefined ? { websiteUrl: body.websiteUrl?.trim() || null } : {}), ...(body.githubUrl !== undefined ? { githubUrl: body.githubUrl?.trim() || null } : {}),
                ...(body.avatarMode !== undefined ? { avatarMode: body.avatarMode } : {}), ...(body.avatarValue !== undefined ? { avatarValue: body.avatarValue?.trim() || null } : {}),
                ...(body.profileBannerUrl !== undefined ? { profileBannerUrl: body.profileBannerUrl?.trim() || null } : {}), ...(body.accentColor !== undefined ? { accentColor: body.accentColor || null } : {}),
            } });
    ...
    fastify.get("/v1/users/:userId/posts", async (request) => {
        const { userId } = request.params as { userId: string }; const q = request.query as Record<string, unknown>; const p = parsePagination(q); const where = { userId };
        const [items, total] = await Promise.all([
            prisma.post.findMany({ where, include: postInclude, skip: p.skip, take: p.limit, orderBy: { createdAt: parseOrder(q.order) } }),
            prisma.post.count({ where }),
        ]);
