...
const publicUserSelect = {
  id: true, name: true, image: true, bio: true, websiteUrl: true, githubUrl: true,
  avatarMode: true, avatarValue: true, profileBannerUrl: true, accentColor: true,
  updatedAt: true, createdAt: true, _count: { select: { posts: true } },
} as const;
...
export const userRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/v1/users", async (request) => {
    ...
    const [users, total] = await Promise.all([prisma.user.findMany({ where, skip: p.skip, take: p.limit, orderBy: { createdAt: parseOrder(q.order) }, select: publicUserSelect }), prisma.user.count({ where })]);
  ...
  fastify.post("/v1/users", { schema: userCreateSchema }, async (request, reply) => {
    ...
    return reply.code(201).send(ok(await prisma.user.create({ data: { id: randomUUID(), name: body.name.trim(), email: body.email.trim(), image: body.image, bio: body.bio?.trim() || undefined, websiteUrl: body.websiteUrl?.trim() || undefined, githubUrl: body.githubUrl?.trim() || undefined } })));
  });
  ...
  fastify.patch("/v1/users/:userId", { schema: userUpdateSchema }, async (request, reply) => {
    ...
    const body = request.body as { name?: string; image?: string | null; bio?: string | null; websiteUrl?: string | null; githubUrl?: string | null; avatarMode?: string; avatarValue?: string | null; profileBannerUrl?: string | null; accentColor?: string | null };
    ...
    const updated = await prisma.user.update({ where: { id: userId }, data: {
      ...(body.name !== undefined ? { name: body.name.trim() } : {}), ...(body.image !== undefined ? { image: body.image } : {}),
      ...(body.bio !== undefined ? { bio: body.bio?.trim() || null } : {}), ...(body.websiteUrl !== undefined ? { websiteUrl: body.websiteUrl?.trim() || null } : {}),
      ...(body.githubUrl !== undefined ? { githubUrl: body.githubUrl?.trim() || null } : {}), ...(body.avatarMode !== undefined ? { avatarMode: body.avatarMode } : {}),
      ...(body.avatarValue !== undefined ? { avatarValue: body.avatarValue?.trim() || null } : {}), ...(body.profileBannerUrl !== undefined ? { profileBannerUrl: body.profileBannerUrl?.trim() || null } : {}),
