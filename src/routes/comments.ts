    ...
    fastify.get(
        "/v1/posts/:postId/comments",
        {
            schema: openapi({
                ...
                description:
                    "Returns comments for a post in chronological order, including whether the current viewer liked each comment.",
    ...
    fastify.patch(
        ...
        async (request, reply) => {
            ...
            if (!comment)
                return reply
                    .code(404)
                    .send({ error: { code: "COMMENT_NOT_FOUND", message: "Comment not found." } });
    ...
    fastify.delete(
        "/v1/comments/:commentId",
        {
            schema: openapi({
                ...
                description:
                    "Deletes a comment owned by the authenticated user or by the owner of its post.",
        ...
        async (request, reply) => {
            ...
            if (comment.userId !== user.id && comment.post.userId !== user.id)
                return reply
                    .code(403)
                    .send({
                        error: { code: "FORBIDDEN", message: "You cannot remove this comment." },
                    });
    ...
    fastify.put(
        "/v1/comments/:commentId/like",
        {
            schema: openapi({
                ...
                description:
                    "Creates the authenticated user's like reaction on a comment. Repeating the operation is idempotent.",
        ...
        async (request, reply) => {
            ...
            await prisma.commentReaction.upsert({
                where: { userId_commentId_type: { userId: user.id, commentId, type: "like" } },
                update: {},
                create: { userId: user.id, commentId, type: "like" },
            });
    ...
    fastify.delete(
        ...
        async (request, reply) => {
            ...
            await prisma.commentReaction.deleteMany({
                where: { userId: user.id, commentId, type: "like" },
            });
            return ok(await commentView(commentId, user.id));
        },
    );
