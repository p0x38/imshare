...
const commentId = parameter.path("commentId", { type: "string" }, { description: "Comment ID." });
const pagination = [
    parameter.query("page", { type: "integer", minimum: 1, default: 1 }, { description: "1-based page number." }),
    parameter.query("limit", { type: "integer", minimum: 1, maximum: 100, default: 20 }, { description: "Maximum number of comments to return." }),
];
const commentBody = {
    type: "object",
    required: ["body"],
...
function view(comment: any) {
    return {
        ...
        author: {
            ...comment.user,
            avatarUrl: `/v1/users/${encodeURIComponent(comment.user.id)}/avatar`,
        },
    ...
    const liked = userId
        ? Boolean(
              await prisma.commentReaction.findUnique({
                  where: { userId_commentId_type: { userId, commentId: id, type: "like" } },
              }),
          )
        : false;
...
export const commentRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get("/v1/posts/:postId/comments", {
        schema: openapi({
            tags: "Comments",
            summary: "List post comments",
            description: "Returns comments for a post in chronological order, including whether the current viewer liked each comment.",
            operationId: "listPostComments",
            parameters: [postId, ...pagination],
            security: [{}, { cookieAuth: [] }],
            responses: {
                "200": { description: "Paginated comment collection." },
        ...
        const liked = session?.user.id
            ? new Set((await prisma.commentReaction.findMany({ where: { userId: session.user.id, commentId: { in: comments.map((c) => c.id) }, type: "like" }, select: { commentId: true } })).map((x) => x.commentId))
            : new Set<string>();
        return collection(comments.map((comment) => ({ ...view(comment), liked: liked.has(comment.id) })), p.page, p.limit, total);
    });

    fastify.post("/v1/posts/:postId/comments", {
        schema: openapi({
            tags: "Comments",
            summary: "Create a comment",
            description: "Creates a comment on a post for the authenticated user.",
            operationId: "createPostComment",
            parameters: [postId],
            requestBody: { required: true, content: { "application/json": { schema: commentBody, example: { body: "Nice post!" } } } },
            security: [{ cookieAuth: [] }],
            responses: {
    ...
    fastify.patch("/v1/comments/:commentId", {
        schema: openapi({
            tags: "Comments",
            summary: "Edit a comment",
            description: "Updates the authenticated user's comment.",
            operationId: "updateComment",
            parameters: [commentId],
            requestBody: { required: true, content: { "application/json": { schema: commentBody } } },
            security: [{ cookieAuth: [] }],
            responses: {
    ...
    fastify.delete("/v1/comments/:commentId", {
        schema: openapi({
            tags: "Comments",
            summary: "Delete a comment",
            description: "Deletes a comment owned by the authenticated user or by the owner of its post.",
    ...
    fastify.put("/v1/comments/:commentId/like", {
        schema: openapi({
            tags: "Comments",
            summary: "Like a comment",
            description: "Creates the authenticated user's like reaction on a comment. Repeating the operation is idempotent.",
            operationId: "likeComment",
            parameters: [commentId],
            security: [{ cookieAuth: [] }],
            responses: {
                "200": { description: "Updated comment state." },
                "401": { $ref: "#/components/responses/Unauthorized" },
                "404": { $ref: "#/components/responses/NotFound" },
            },
        }),
    ...
    fastify.delete("/v1/comments/:commentId/like", {
        schema: openapi({
            tags: "Comments",
            summary: "Unlike a comment",
            description: "Removes the authenticated user's like reaction from a comment.",
