...
export const adminRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get("/v1/admin/overview", async (request, reply) => {
    ...
    const [users, posts, openReports, bannedUsers, admins, moderators] = await Promise.all([
      prisma.user.count(), prisma.post.count(), prisma.report.count({ where: { status: "open" } }),
      prisma.user.count({ where: { isBanned: true } }), prisma.user.count({ where: { role: "admin" } }),
  ...
  fastify.get("/v1/admin/users", async (request, reply) => {
    ...
    const where = { ...(search ? { OR: [{ name: { contains: search } }, { email: { contains: search } }] } : {}), ...(role ? { role } : {}), ...(banned !== undefined ? { isBanned: banned } : {}) };
    const [users, total] = await Promise.all([
      prisma.user.findMany({ where, skip: (page - 1) * limit, take: limit, orderBy: { createdAt: "desc" }, select: {
        id: true, name: true, email: true, role: true, isPublic: true, showEmail: true, showPosts: true, showProfile: true,
        isBanned: true, banReason: true, bannedAt: true, bannedUntil: true, createdAt: true, updatedAt: true,
      }}),
  ...
  fastify.get("/v1/admin/reports", async (request, reply) => {
    ...
    const [reports, total] = await Promise.all([
      prisma.report.findMany({ where, skip: (page - 1) * limit, take: limit, orderBy: { createdAt: "desc" }, include: {
        reporter: { select: { id: true, name: true, email: true } },
        post: { select: { id: true, title: true, userId: true } },
        comment: { select: { id: true, body: true, userId: true, postId: true } },
      }}),
  ...
  fastify.post("/v1/admin/users/:userId/ban", async (request, reply) => {
    ...
    await prisma.$transaction([
      prisma.user.update({ where: { id: userId }, data: { isBanned: true, banReason: reason, bannedAt: now, bannedUntil } }),
      prisma.session.deleteMany({ where: { userId } }),
      prisma.moderationLog.create({ data: { actorId: actor.id, targetUserId: userId, action: "ban", reason, expiresAt: bannedUntil ?? undefined } }),
    ]);
  ...
  fastify.get("/v1/admin/logs", async (request, reply) => {
    const actor = await requireRole(request, reply, "moderator");
    if (!actor) return;
    const logs = await prisma.moderationLog.findMany({ take: 100, orderBy: { createdAt: "desc" }, include: {
      actor: { select: { id: true, name: true } }, targetUser: { select: { id: true, name: true } },
    }});
    return ok(logs);
  });
};
