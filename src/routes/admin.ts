...
export const adminRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.patch("/v1/admin/users/:userId/role", async (request, reply) => {
        ...
        if (!isUserRole(body.role))
            return reply
                .code(400)
                .send({ error: { code: "INVALID_ROLE", message: "Invalid user role." } });
        ...
        if (
            ...
            return reply
                .code(400)
                .send({
                    error: { code: "LAST_ADMIN", message: "The last admin cannot be demoted." },
                });
        ...
        await prisma.moderationLog.create({
            data: {
                ...
                targetUserId: userId,
                action: "role_change",
                reason: `Role changed from ${target.role} to ${body.role}.`,
            },
        });
