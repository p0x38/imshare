...
export const postRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get("/v1/posts", async (request) => {
    ...
    if (typeof q.search === "string") where.OR = [{ title: { contains: q.search } }, { description: { contains: q.search } }, { caption: { contains: q.search } }];
  ...
  fastify.post("/v1/posts", { schema: postCreateSchema }, async (request, reply) => {
    ...
    if (!user) return;
    const body = request.body as { title: string; description?: string; caption?: string; sourceUrl?: string; allowDownload?: boolean; tags?: string[]; categoryId?: string | null; uploadIds?: string[] };
    ...
    if (uploadIds.length > 0) {
      ...
      if (ownedUploads !== uploadIds.length) return reply.code(403).send({ error: { code: "FORBIDDEN", message: "You can only attach your own unused uploads." } });
    }
    const tags = await findTags(body.tags ?? []);
    const post = await prisma.post.create({
      data: { title: body.title.trim(), description: body.description, caption: body.caption, sourceUrl: body.sourceUrl, allowDownload: body.allowDownload ?? true, categoryId: body.categoryId, userId: user.id, tags: { create: tags.map((tag) => ({ tagId: tag.id })) }, uploads: uploadIds.length ? { connect: uploadIds.map((id) => ({ id })) } : undefined },
      include: postInclude,
    });
  ...
  fastify.patch("/v1/posts/:postId", { schema: postUpdateSchema }, async (request, reply) => {
    ...
    const body = request.body as { title?: string; description?: string | null; caption?: string | null; sourceUrl?: string | null; allowDownload?: boolean; categoryId?: string | null; tags?: string[] };
    ...
    const post = await prisma.post.update({ where: { id: postId }, data: { title: body.title?.trim(), description: body.description, caption: body.caption, sourceUrl: body.sourceUrl, allowDownload: body.allowDownload, categoryId: body.categoryId, ...(body.tags ? { tags: { create: tags.map((tag) => ({ tagId: tag.id })) } } : {}) }, include: postInclude });
    return ok(postView(post));
  });
  ...
  fastify.get("/v1/posts/:postId/tags", async (request) => { const { postId } = request.params as { postId: string }; return ok(await prisma.postTag.findMany({ where: { postId }, include: { tag: true } }).then((items) => items.map((item) => item.tag))); });
  fastify.post("/v1/posts/:postId/tags", { schema: postTagSchema }, async (request, reply) => { const user = await requireUser(request, reply); if (!user) return; const { postId } = request.params as { postId: string }; const body = request.body as { tagId: string }; const post = await prisma.post.findUnique({ where: { id: postId } }); if (!post) return reply.code(404).send({ error: { code: "POST_NOT_FOUND", message: "Post not found." } }); if (post.userId !== user.id) return reply.code(403).send({ error: { code: "FORBIDDEN", message: "You do not own this post." } }); const relation = await prisma.postTag.upsert({ where: { postId_tagId: { postId, tagId: body.tagId } }, update: {}, create: { postId, tagId: body.tagId }, include: { tag: true } }); return reply.code(201).send(ok(relation.tag)); });
