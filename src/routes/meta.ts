...
export const metaRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get("/sitemap.xml", async (_request, reply) => {
        const [users, posts, tags, categories] = await Promise.all([
            ...
            prisma.post.findMany({
                ...
                select: {
                    id: true,
                    updatedAt: true,
                    uploads: { orderBy: { createdAt: "asc" }, take: 1000, select: { id: true } },
                },
            }),
        ...
        const entries = [
            ...
            ...users.map((user) => {
                const pathname = user.handle ? `/users/@${encodeURIComponent(user.handle)}` : `/users/${encodeURIComponent(user.id)}`;
