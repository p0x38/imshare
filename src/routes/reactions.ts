...
const stateResponse = { description: "Current reaction counts and the authenticated user's active reactions." };

export const reactionRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get("/v1/posts/:postId/reactions", {
        schema: openapi({
            tags: "Reactions",
            summary: "Get post reactions",
            description: "Returns reaction counts and, when authenticated, the current user's active reactions.",
            ...
            security: [{}, { cookieAuth: [] }],
            responseExamples: {
                "200": { data: { counts: { like: 12, favorite: 3, save: 7 }, active: { like: false, favorite: true, save: false } } },
    ...
    fastify.put("/v1/posts/:postId/:type", {
        schema: openapi({
            tags: "Reactions",
            summary: "Add post reaction",
            description: "Activates one of the supported reactions for the authenticated user. The operation is idempotent.",
            ...
            responseExamples: {
                "200": { data: { counts: { like: 13, favorite: 3, save: 7 }, active: { like: true, favorite: false, save: false } } },
                "400": { error: { code: "INVALID_REACTION", message: "Supported reactions are like, favorite, and save." } },
            },
        }),
    }, async (request, reply) => {
        ...
        if (!isReactionType(type))
            return reply.code(400).send({ error: { code: "INVALID_REACTION", message: "Supported reactions are like, favorite, and save." } });
        const post = await prisma.post.findUnique({ where: { id: postId }, select: { id: true, userId: true, title: true } });
        if (!post)
            return reply.code(404).send({ error: { code: "POST_NOT_FOUND", message: "Post not found." } });
    ...
    fastify.delete("/v1/posts/:postId/:type", {
        schema: openapi({
            tags: "Reactions",
            summary: "Remove post reaction",
            description: "Removes the selected reaction for the authenticated user. The operation is idempotent.",
            ...
            responseExamples: {
                "200": { data: { counts: { like: 12, favorite: 3, save: 7 }, active: { like: false, favorite: false, save: false } } },
                "404": { error: { code: "POST_NOT_FOUND", message: "Post not found." } },
            },
        }),
    }, async (request, reply) => {
