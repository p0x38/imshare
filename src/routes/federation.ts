...
function asActor(baseUrl: string, user: { id: string; name: string; handle: string; image: string | null }, publicKeyPem: string) {
...
function postActivity(baseUrl: string, actor: string, post: { id: string; title: string; description: string | null; createdAt: Date; publishedAt: Date | null }) {
...
async function handleActivity(baseUrl: string, activity: Record<string, unknown>, recipientUserId: string) {
    ...
    if (type === "Follow") {
        const followerInbox = typeof activity.actor === "object" && activity.actor !== null ? firstString((activity.actor as Record<string, unknown>).inbox) : undefined;
        const actorData = await (await fetch(actor, { headers: { accept: "application/activity+json, application/ld+json" }, redirect: "error", signal: AbortSignal.timeout(10_000) })).json() as { inbox?: string; endpoints?: { sharedInbox?: string } };
        const inbox = followerInbox ?? actorData.inbox;
        if (!inbox) throw new Error("Follower actor has no inbox.");
        await addFollower(recipientUserId, { actorId: actor, inbox, sharedInbox: actorData.endpoints?.sharedInbox });
        return;
    }
    if (type === "Undo") {
        const object = activity.object;
        if (typeof object === "object" && object !== null && (object as Record<string, unknown>).type === "Follow") await removeFollower(recipientUserId, actor);
        return;
    }
...
export const federationRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.get("/.well-known/webfinger", async (request, reply) => {
        ...
        if (!resource) return reply.code(400).send({ error: { code: "WEBFINGER_RESOURCE_REQUIRED", message: "The resource query parameter is required." } });
        ...
        if (resource.toLowerCase().startsWith("acct:")) {
            ...
            try {
                ...
                if (url.origin === baseUrl) handle = url.pathname.match(/^\/users\/@([^/]+)\/?$/)?.[1] ?? null;
            } catch {
        ...
        return reply.type(JRD_JSON).header("Access-Control-Allow-Origin", "*").send({ subject, aliases: [profileUrl(baseUrl, user.handle)], links });
    });

    fastify.get("/.well-known/nodeinfo", async (_request, reply) => reply.type(JRD_JSON).header("Access-Control-Allow-Origin", "*").send({ links: [{ rel: "http://nodeinfo.diaspora.software/ns/schema/2.1", href: `${baseUrl}/nodeinfo/2.1` }] }));
    ...
    fastify.get("/nodeinfo/2.1", async (_request, reply) => {
        ...
        const [users, activeMonth, localPosts, localComments] = await Promise.all([
            prisma.user.count(), prisma.user.count({ where: { updatedAt: { gte: since } } }),
            prisma.post.count({ where: { status: "published", visibility: "public" } }), prisma.comment.count(),
        ]);
        return reply.type("application/json").send({ version: "2.1", software: { name: "imshare", version: config.site.version, repository: "https://github.com/p0x38/imshare" }, protocols: ["activitypub"], services: { inbound: ["atom"], outbound: ["atom"] }, usage: { users: { total: users, activeMonth }, localPosts, localComments }, openRegistrations: config.auth.registration?.public === true, metadata: { siteName: config.site.name } });
    });

    fastify.get("/federation/actors/:handle", async (request, reply) => {
        const { handle } = request.params as { handle: string };
        const local = await localActor(baseUrl, handle);
        if (!local) return reply.code(404).send({ error: { code: "ACTOR_NOT_FOUND", message: "The requested actor was not found." } });
    ...
    fastify.get("/federation/actors/:handle/outbox", async (request, reply) => {
        ...
        const local = await localActor(baseUrl, handle);
        if (!local) return reply.code(404).send({ error: { code: "ACTOR_NOT_FOUND", message: "The requested actor was not found." } });
        const posts = await prisma.post.findMany({ where: { userId: local.user.id, status: "published", visibility: "public" }, orderBy: { publishedAt: "desc" }, take: 20, select: { id: true, title: true, description: true, createdAt: true, publishedAt: true } });
        const items = posts.map((post) => postActivity(baseUrl, local.actor, post));
        return reply.type(ACTIVITY_JSON).send({ "@context": ACTIVITY_STREAMS, id: `${local.actor}/outbox`, type: "OrderedCollection", totalItems: items.length, orderedItems: items });
    });
    ...
    fastify.post("/federation/actors/:handle/inbox", async (request, reply) => {
        ...
        try {
            ...
            if (activity.type === "Follow" && typeof activity.actor === "string") {
                const accept = { "@context": ACTIVITY_STREAMS, id: `${local.actor}#accept-${Date.now()}`, type: "Accept", actor: local.actor, object: activity };
                const followerResponse = await (await fetch(verified.actor.id ?? verified.keyId, { headers: { accept: "application/activity+json" }, redirect: "error", signal: AbortSignal.timeout(10_000) })).json() as { inbox?: string };
                if (followerResponse.inbox) await deliverActivity({ inbox: followerResponse.inbox, actorUrl: local.actor, activity: accept, privateKeyPem: local.key.privateKeyPem });
            }
            return reply.code(202).send({ data: { accepted: true } });
        } catch (error) {
            request.log.warn({ err: error }, "Rejected federation activity");
            return reply.code(401).send({ error: { code: "FEDERATION_REQUEST_REJECTED", message: error instanceof Error ? error.message : "Federation request rejected." } });
        }
    });

    fastify.post("/federation/deliver", async (request, reply) => {
        const session = await import("../lib/api.js").then(({ getSession }) => getSession(request as FastifyRequest));
        ...
        if (!body?.handle || !body.inbox || body.activity === undefined) return reply.code(400).send({ error: { code: "INVALID_DELIVERY", message: "handle, inbox and activity are required." } });
        const local = await localActor(baseUrl, body.handle);
        if (!local || local.user.id !== session.user.id) return reply.code(403).send({ error: { code: "FORBIDDEN", message: "You may only deliver as yourself." } });
        const result = await deliverActivity({ inbox: body.inbox, actorUrl: local.actor, activity: body.activity, privateKeyPem: local.key.privateKeyPem });
        return result.ok ? { data: result } : reply.code(502).send({ error: { code: "FEDERATION_DELIVERY_FAILED", message: `Remote server returned HTTP ${result.status}.` }, data: result });
    });
};
