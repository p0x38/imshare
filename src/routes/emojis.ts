...
const emojiBody = {
    ...
    properties: {
        ...
        url: {
            type: "string",
            format: "uri",
            description: "An imshare upload URL under /v1/posts/image/.",
        },
    },
    ...
    fastify.post(
        "/v1/emojis",
        {
            schema: openapi({
                ...
                description:
                    "Creates a custom emoji from an image upload owned by the authenticated user.",
        ...
        async (request, reply) => {
            ...
            try {
                return reply
                    .code(201)
                    .send(
                        ok(await prisma.emoji.create({ data: { name, url, creatorId: user.id } })),
                    );
            } catch (error: any) {
                if (error?.code === "P2002")
                    return reply
