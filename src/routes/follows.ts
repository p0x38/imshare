...
async function resolveUser(value: string) {
    if (value.startsWith("@"))
        return prisma.user.findFirst({ where: { handle: value.slice(1).trim().toLowerCase() } });
...
export const followRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get(
        "/v1/users/:userId/profile",
        {
            schema: openapi({
                ...
                description:
                    "Returns a public profile and follow state when visible to the requester.",
        ...
        async (request, reply) => {
            ...
            const user = await resolveUser(userId);
            if (!user || !user.showProfile || user.isBanned)
                return reply
                    .code(404)
            ...
            const [posts, followers, following, follow] = await Promise.all([
                user.showPosts
                    ? prisma.post.count({ where: { userId: user.id, ...publicPostWhere } })
                    : Promise.resolve(0),
                user.showFollowers
                    ? prisma.follow.count({ where: { followingId: user.id, status: "approved" } })
                    : Promise.resolve(null),
                user.showFollowings
                    ? prisma.follow.count({ where: { followerId: user.id, status: "approved" } })
                    : Promise.resolve(null),
            ...
            return ok({
                id: user.id,
                name: user.name,
                handle: user.showHandle ? user.handle : null,
                badges: userBadges(user),
                bio: user.bio,
                ...
                profileLinks: await prisma.profileLink.findMany({
    ...
    fastify.get(
        "/v1/users/:userId/followers",
        {
            schema: openapi({
                ...
                description:
                    "Lists approved followers when the profile owner allows followers to be visible.",
                operationId: "listUserFollowers",
                parameters: [userId, ...pagination],
                security: [{}, { cookieAuth: [] }],
                responses: {
                    "200": { description: "Follower collection." },
        ...
        async (request, reply) => {
            const { userId: value } = request.params as { userId: string };
            const result = await visibleRelationshipUser(value, "followers");
            if (!result)
                return reply
                    .code(404)
                    .send({ error: { code: "USER_NOT_FOUND", message: "User not found." } });
            if (!result.visible)
                return reply
                    ...
                    .send({
                        error: {
                            code: "FOLLOWERS_HIDDEN",
                            message: "This user does not allow others to see their followers.",
                        },
                    });
            ...
            const [items, total] = await Promise.all([
                prisma.follow.findMany({
                    where: { followingId: result.user.id, status: "approved" },
                    include: {
                        follower: {
                            select: {
                                id: true,
                ...
                prisma.follow.count({ where: { followingId: result.user.id, status: "approved" } }),
            ]);
            return {
                data: items.map((item) => publicFollowUser(item.follower)),
                page,
                limit,
                total,
    ...
    fastify.get(
        "/v1/users/:userId/following",
        {
            schema: openapi({
                ...
                description:
                    "Lists approved following relationships when the profile owner allows following lists to be visible.",
                operationId: "listUserFollowing",
                parameters: [userId, ...pagination],
                security: [{}, { cookieAuth: [] }],
                responses: {
                    "200": { description: "Following collection." },
                    "403": { $ref: "#/components/responses/Forbidden" },
                    "404": { $ref: "#/components/responses/NotFound" },
                },
            }),
        ...
        async (request, reply) => {
            const { userId: value } = request.params as { userId: string };
            const result = await visibleRelationshipUser(value, "following");
            if (!result)
                return reply
                    .code(404)
                    .send({ error: { code: "USER_NOT_FOUND", message: "User not found." } });
            if (!result.visible)
                return reply
                    ...
                    .send({
                        error: {
                            code: "FOLLOWING_HIDDEN",
                            message: "This user does not allow others to see who they follow.",
                        },
                    });
            const q = request.query as Record<string, unknown>;
            const page = Math.max(1, Number(q.page) || 1);
            ...
            const [items, total] = await Promise.all([
                prisma.follow.findMany({
                    where: { followerId: result.user.id, status: "approved" },
                    include: {
                        following: {
                            select: {
                                id: true,
                ...
                prisma.follow.count({ where: { followerId: result.user.id, status: "approved" } }),
            ]);
            return {
                data: items.map((item) => publicFollowUser(item.following)),
                page,
                limit,
                total,
                totalPages: Math.ceil(total / limit),
    ...
    fastify.post(
        "/v1/users/:userId/follow",
        {
            schema: openapi({
                ...
                description:
                    "Creates a follow relationship or follow request for the authenticated user.",
                operationId: "followUser",
                parameters: [userId],
                security: [{ cookieAuth: [] }],
                responses: {
                    "200": { description: "Follow state." },
        ...
        async (request, reply) => {
            ...
            if (!me) return;
            const { userId: value } = request.params as { userId: string };
            const target = await resolveUser(value);
            if (!target || !target.showProfile || target.isBanned)
                return reply
                    .code(404)
                    .send({ error: { code: "USER_NOT_FOUND", message: "User not found." } });
            if (target.id === me.id)
                ...
                return reply
                    ...
                    .send({
                        error: {
                            ...
                            message: "This profile does not accept follows.",
                        },
                    });
            ...
            if (existing)
                return ok({ following: existing.status === "approved", status: existing.status });
    ...
    fastify.delete(
        "/v1/users/:userId/follow",
        {
            schema: openapi({
                ...
                description: "Removes the authenticated user's follow relationship with a user.",
                operationId: "unfollowUser",
                parameters: [userId],
                security: [{ cookieAuth: [] }],
                responses: {
                    "200": { description: "Follow state after removal." },
                    "401": { $ref: "#/components/responses/Unauthorized" },
                    "404": { $ref: "#/components/responses/NotFound" },
                },
            }),
    ...
    fastify.get(
        "/v1/me/follow-requests",
        {
            schema: openapi({
                ...
                description: "Lists pending follow requests received by the authenticated user.",
                operationId: "listFollowRequests",
                security: [{ cookieAuth: [] }],
                responses: {
                    "200": { description: "Pending follow requests." },
                    "401": { $ref: "#/components/responses/Unauthorized" },
                },
            }),
        },
    ...
    fastify.post(
        "/v1/me/follow-requests/:userId/approve",
        {
            schema: openapi({
                tags: "Follows",
                summary: "Approve a follow request",
                description: "Approves a pending follow request from a user.",
        ...
        async (request, reply) => {
            ...
            const result = await prisma.follow.updateMany({
                where: { followerId: value, followingId: me.id, status: "pending" },
    ...
    fastify.delete(
        "/v1/me/follow-requests/:userId",
        {
            schema: openapi({
                ...
                description: "Rejects a pending follow request from a user.",
