...
async function resolveUser(value: string) {
    if (value.startsWith("@")) {
        const handle = value.slice(1).trim().toLowerCase();
        return prisma.user.findFirst({ where: { handle } });
    }
...
export const followRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.post("/v1/users/:userId/follow", async (request, reply) => {
        ...
        if (!target || !target.showProfile || target.isBanned)
            return reply.code(404).send({ error: { code: "USER_NOT_FOUND", message: "User not found." } });
        if (target.id === me.id)
            return reply.code(400).send({ error: { code: "CANNOT_FOLLOW_SELF", message: "You cannot follow yourself." } });
        if (!target.isPublic)
            return reply.code(403).send({ error: { code: "PROFILE_PRIVATE", message: "This profile does not accept follows." } });
