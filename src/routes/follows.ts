...
export const followRoutes: FastifyPluginAsync = async (fastify) => {
    fastify.get("/v1/users/:userId/profile", {
        schema: openapi({ tags: "Users", summary: "Get a user profile", description: "Returns a public profile and follow state when visible to the requester.", operationId: "getUserProfile", parameters: [userId], security: [{}, { cookieAuth: [] }], responses: { "200": { description: "User profile." }, "404": { $ref: "#/components/responses/NotFound" } } }),
    }, async (request, reply) => {
    ...
    fastify.get("/v1/me/follow-requests", {
        schema: openapi({ tags: "Follows", summary: "List follow requests", description: "Lists pending follow requests received by the authenticated user.", operationId: "listFollowRequests", security: [{ cookieAuth: [] }], responses: { "200": { description: "Pending follow requests." }, "401": { $ref: "#/components/responses/Unauthorized" } } }),
    }, async (request, reply) => {
        const me = await requireUser(request, reply); if (!me) return;
        const requests = await prisma.follow.findMany({ where: { followingId: me.id, status: "pending" }, orderBy: { createdAt: "desc" }, include: { follower: { select: { id: true, name: true, handle: true, image: true, createdAt: true } } } });
        return ok(requests);
    });

    fastify.post("/v1/me/follow-requests/:userId/approve", {
        schema: openapi({ tags: "Follows", summary: "Approve a follow request", description: "Approves a pending follow request from a user.", operationId: "approveFollowRequest", parameters: [userId], security: [{ cookieAuth: [] }], responses: { "200": { description: "Request approved." }, "401": { $ref: "#/components/responses/Unauthorized" }, "404": { $ref: "#/components/responses/NotFound" } } }),
    }, async (request, reply) => {
        const me = await requireUser(request, reply); if (!me) return;
        const { userId } = request.params as { userId: string };
