...
export function TagAutocomplete({ value, onChange }: { value: string; onChange: (value: string) => void }) {
    ...
    useEffect(() => {
        ...
        const timer = window.setTimeout(() => {
            setLoading(true);
            void api<{ data?: TagOption[] }>(`/v1/tags/autocomplete?q=${encodeURIComponent(term)}&limit=10`)
    ...
    return <Autocomplete<TagOption, true, false, true>
        ...
        value={selectedValues.map((name) => ({ id: `value:${name}`, name, slug: name, postCount: 0 }))}
        ...
        renderOption={(props, option) => <Box component="li" {...props} key={option.id}>
            <Box sx={{ minWidth: 0 }}>
                ...
                {option.postCount > 0 ? <Typography variant="caption" color="text.secondary">{option.postCount} post{option.postCount === 1 ? "" : "s"}</Typography> : null}
            </Box>
        </Box>}
...
export function SearchAutocomplete({ value, onChange, onSubmit, fullWidth = false }: {
    value: string;
        ...
        const timer = window.setTimeout(() => {
            ...
            void api<SearchResponse>(`/v1/search?q=${encodeURIComponent(term)}&type=all&limit=8`)
                .then((response) => {
                    ...
                    setOptions(suggestions.filter((suggestion, index, all) => all.findIndex((candidate) => candidate.label.toLocaleLowerCase() === suggestion.label.toLocaleLowerCase()) === index).slice(0, 8));
                })
                .catch(() => setOptions([]))
                .finally(() => setLoading(false));
    ...
    return <Autocomplete<SearchSuggestion, false, false, true>
        freeSolo
        fullWidth={fullWidth}
        options={options}
        value={null}
        ...
        renderOption={(props, option) => <Box component="li" {...props} key={`${option.type}:${option.label}`}>
            <Typography variant="body2">{option.label}</Typography>
            <Typography variant="caption" color="text.secondary" sx={{ ml: 1 }}>{option.type}</Typography>
        </Box>}
        renderInput={(params) => <TextField
            {...params}
            label="Search"
