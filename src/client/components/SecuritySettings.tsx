...
export function SecuritySettings() {
    ...
    async function enable() {
        ...
        if (result.error) setError(result.error.message || "Unable to start TOTP enrollment.");
            ...
            setNotice(
                "Scan the TOTP URI with your authenticator app, then enter the generated code below.",
            );
        }
        setLoading(false);
    }
    ...
    async function verify() {
        ...
        if (result.error)
            setError(result.error.message || "Unable to verify the authenticator code.");
        else {
            setEnabled(true);
    ...
    async function regenerate() {
        ...
        if (result.error) setError(result.error.message || "Unable to generate recovery codes.");
            ...
            setBackupCodes(result.data.backupCodes);
            setNotice("New recovery codes generated. The previous codes are no longer valid.");
        }
        setLoading(false);
    }
                ...
                <CardContent>
                    <Stack spacing={2}>
                        ...
                        <Typography variant="body2" color="text.secondary">
                            Use an authenticator app for sign-in verification. Recovery codes can be
                            used if you lose access to the authenticator.
                        </Typography>
                        <TextField
                        ...
                        {totpUri ? (
                            <>
                                <TextField
                                    label="TOTP setup URI"
                                    value={totpUri}
                                    slotProps={{ input: { readOnly: true } }}
                                    multiline
                                    minRows={3}
                                    helperText="Keep this URI private. It contains the authenticator secret."
                                />
                                <TextField
                        ...
                        {backupCodes.length ? (
                            <Alert severity="warning">
                                <Stack spacing={1}>
                                    <Typography>
                                        Save these recovery codes somewhere secure. Each code can be
                                        used once.
                                    </Typography>
