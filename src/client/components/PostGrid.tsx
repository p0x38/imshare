    ...
    return <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 256px), 1fr))", gap: { xs: 1, sm: 2 } }}>
        {posts.map((post, index) => <AnimatedItem key={post.id} delay={Math.min(index, 10) * 35}><Card variant="outlined" sx={{ overflow: "hidden", height: "100%", transition: (theme) => theme.transitions.create(["transform", "box-shadow"], { duration: 220 }), "@media (hover: hover)": { "&:has(.MuiCardActionArea-root:hover)": { transform: "translateY(-3px)", boxShadow: 3 } }, "@media (prefers-reduced-motion: reduce)": { transition: "none" } }}>
            <CardActionArea component="a" href={`/posts/${encodeURIComponent(post.id)}`} sx={{ display: "block", textAlign: "left", height: "100%" }}>
                {post.uploads?.[0] ? <CardMedia component="img" image={imageUrl(post.uploads[0].url)} alt={post.uploads[0].alt || post.title || ""} loading="lazy" sx={{ aspectRatio: "1 / 1", objectFit: "cover", transition: (theme) => theme.transitions.create("transform", { duration: 320 }), "@media (hover: hover)": { ".MuiCardActionArea-root:hover &": { transform: "scale(1.025)" } }, "@media (prefers-reduced-motion: reduce)": { transition: "none" } }} /> : null}
                <CardContent sx={{ p: { xs: 1.25, sm: 2 }, "&:last-child": { pb: { xs: 1.25, sm: 2 } } }}><Typography variant="subtitle1" noWrap sx={{ fontSize: { xs: "0.9rem", sm: "1rem" } }}>{post.title || t("common.untitled")}</Typography><Typography variant="body2" color="text.secondary" noWrap sx={{ fontSize: { xs: "0.75rem", sm: "0.875rem" } }}>{post.author?.name || post.author?.username || post.authorName || t("common.unknownAuthor")}</Typography></CardContent>
            </CardActionArea>
        </Card></AnimatedItem>)}
    </Box>;
}
