...
const defaultThemeSettings: ThemeSettings = { animations: false, ripple: false, accentColor: "default" };
interface ColorModeContextValue { mode: ColorMode; preference: ColorModePreference; setPreference: (preference: ColorModePreference) => void; toggle: () => void; }
interface LanguageContextValue { language: Language; setLanguage: (language: Language) => void; }
interface ThemeSettingsContextValue { settings: ThemeSettings; setAnimations: (enabled: boolean) => void; setRipple: (enabled: boolean) => void; setAccentColor: (color: AccentColor) => void; }
...
function initialThemeSettings(): ThemeSettings { try { const stored = window.localStorage.getItem("imshare-theme-settings"); if (!stored) return defaultThemeSettings; const parsed = JSON.parse(stored) as Partial<ThemeSettings>; const accentColor: AccentColor = parsed.accentColor === "blue" || parsed.accentColor === "purple" || parsed.accentColor === "green" || parsed.accentColor === "orange" ? parsed.accentColor : "default"; return { animations: parsed.animations === true, ripple: parsed.ripple === true, accentColor }; } catch { return defaultThemeSettings; } }
            ...
            <LanguageContext.Provider value={languageContext}>
                <ThemeSettingsContext.Provider value={themeSettings}>
                    <ThemeProvider theme={theme}>
                        <CssBaseline />
                        <GlobalStyles styles={{ "main:has(> [id$='-page']), main#post": { width: "100% !important", maxWidth: "none !important", margin: "0 !important", padding: "0 !important" } }} />
                        {children}
                        {location.pathname === "/account/" ? <AccountPreferences /> : null}
                    </ThemeProvider>
