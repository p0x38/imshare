...
export function Navigation() {
    ...
    useEffect(() => {
        ...
        void Promise.allSettled([
            ...
            if (notificationResult.status === "fulfilled")
                setUnreadNotifications(notificationResult.value.data.count);
        });
        return () => {
            active = false;
        };
    ...
    return (
        <AppBar position="static" elevation={0}>
            ...
            >
                <IconButton color="inherit" edge="start" aria-label={t("nav.openNavigation")} onClick={() => setDrawerOpen(true)} sx={{ display: { xs: "inline-flex", sm: "none" } }}>
                    <Menu />
                </IconButton>
                <Typography component="a" href="/" variant="h6" sx={{ mr: { xs: 0, sm: 1 }, color: "inherit", textDecoration: "none", fontWeight: 700, flexShrink: 0 }}>
                    imshare
                </Typography>
                ...
                <Stack direction="row" spacing={0.25} alignItems="center" sx={{ ml: "auto" }}>
                    ...
                    <Tooltip title={unreadNotifications > 0 ? t("nav.unreadNotifications", { count: unreadNotifications }) : t("nav.notifications")}>
                        <IconButton component="a" href="/notifications/" color="inherit" aria-label={t("nav.notifications")}>
                            <Badge badgeContent={unreadNotifications} color="secondary" max={99} showZero={false}><NotificationsOutlined /></Badge>
                        </IconButton>
                    </Tooltip>
                    <Tooltip title={displayName}>
                        <IconButton color="inherit" aria-label={t("nav.account", { name: displayName })} aria-controls={accountMenuOpen ? "account-menu" : undefined} aria-haspopup="menu" aria-expanded={accountMenuOpen ? "true" : undefined} onClick={(event) => setAccountMenuAnchor(event.currentTarget)}>
                            <Avatar alt={displayName} src={user?.avatarUrl ?? undefined} sx={{ width: 32, height: 32 }}>{user ? initials : <AccountCircle />}</Avatar>
                        </IconButton>
                    </Tooltip>
                    <MuiMenu id="account-menu" anchorEl={accountMenuAnchor} open={accountMenuOpen} onClose={closeAccountMenu} anchorOrigin={{ vertical: "bottom", horizontal: "right" }} transformOrigin={{ vertical: "top", horizontal: "right" }} slotProps={{ paper: { sx: { minWidth: 220, mt: 1 } } }}>
            ...
            <Drawer anchor="left" open={drawerOpen} onClose={() => setDrawerOpen(false)} ModalProps={{ keepMounted: true }} PaperProps={{ sx: { width: { xs: "min(82vw, 320px)", sm: 320 }, pt: "env(safe-area-inset-top)" } }}>
                <Stack direction="row" alignItems="center" justifyContent="space-between" px={2} py={1.25}>
                    <Typography variant="h6" fontWeight={700}>imshare</Typography>
                    <IconButton aria-label={t("nav.closeNavigation")} onClick={() => setDrawerOpen(false)}><Close /></IconButton>
                </Stack>
                <List sx={{ px: 1 }}>
                    {links.map(([key, href]) => <ListItemButton key={href} component="a" href={href} onClick={() => setDrawerOpen(false)} sx={{ minHeight: 48, borderRadius: 1.5 }}><ListItemText primary={label(key)} /></ListItemButton>)}
                    {canAccessAdmin ? <ListItemButton component="a" href="/admin/" onClick={() => setDrawerOpen(false)} sx={{ minHeight: 48, borderRadius: 1.5 }}><ListItemText primary={t("admin.administration")} /></ListItemButton> : null}
                </List>
            </Drawer>
        </AppBar>
    );
