...
type RouteKind =
    | "login" | "register" | "notifications" | "profile" | "users" | "user" | "user-posts"
    | "tags" | "tag" | "tag-posts" | "categories" | "category" | "category-posts" | "search"
...
function PageCards({ posts }: { posts: Post[] }) { return <PostGrid posts={posts} />; }
...
function AuthPage({ register }: { register: boolean }) {
    ...
    async function submit(event: React.FormEvent) {
        ...
        try { await api(register ? "/v1/auth/sign-up/email" : "/v1/auth/sign-in/email", { method: "POST", body: JSON.stringify(register ? { name, email, password, registrationToken: token.trim() } : { email, password }) }); location.href = "/dashboard/"; }
        catch (cause) { setError(cause instanceof Error ? cause.message : register ? "Registration failed." : "Login failed."); }
...
function SearchPage() {
    const [query, setQuery] = useState(new URLSearchParams(location.search).get("q") ?? ""); const [submitted, setSubmitted] = useState(query); const [posts, setPosts] = useState<Post[]>([]); const [error, setError] = useState("");
    useEffect(() => { if (!submitted.trim()) { setPosts([]); return; } void api<{ data?: Post[] }>(`/v1/search?q=${encodeURIComponent(submitted.trim())}&type=all&limit=25`).then((r) => setPosts(r.data ?? [])).catch((cause) => setError(cause instanceof Error ? cause.message : "Search failed.")); }, [submitted]);
    return <Page><Stack spacing={2}><Typography variant="h4" component="h1">Search</Typography><Stack component="form" direction="row" spacing={1} onSubmit={(e) => { e.preventDefault(); setSubmitted(query); }}><TextField fullWidth label="Search" value={query} onChange={(e) => setQuery(e.target.value)} /><Button type="submit" variant="contained">Search</Button></Stack>{error ? <Alert severity="error">{error}</Alert> : null}<PageCards posts={posts} /></Stack></Page>;
}

function StaticPage({ title, children }: { title: string; children: React.ReactNode }) { return <Page maxWidth="md"><Stack spacing={2}><Typography variant="h4" component="h1">{title}</Typography><Card variant="outlined"><CardContent><Stack spacing={2}>{children}</Stack></CardContent></Card></Stack></Page>; }

function AdminPage() {
    const [overview, setOverview] = useState<Record<string, unknown> | null>(null); const [token, setToken] = useState(""); const [error, setError] = useState("");
    useEffect(() => { void api<{ data: Record<string, unknown> }>("/v1/admin/overview").then((r) => setOverview(r.data)).catch((cause) => setError(cause instanceof Error ? cause.message : "Unable to load admin overview.")); }, []);
    async function rotateToken() { try { const r = await api<{ data?: { token?: string } }>("/v1/admin/registration-token", { method: "POST" }); setToken(r.data?.token ?? ""); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to rotate token."); } }
    return <Page><Stack spacing={2}><Typography variant="h4" component="h1">Admin</Typography>{error ? <Alert severity="error">{error}</Alert> : null}{overview ? <Card variant="outlined"><CardContent><Stack spacing={1}>{Object.entries(overview).map(([key, value]) => <Typography key={key}><strong>{key}:</strong> {typeof value === "object" ? JSON.stringify(value) : String(value)}</Typography>)}</Stack></CardContent></Card> : <LoadingState />}{token ? <Alert severity="warning">New registration token: <code>{token}</code></Alert> : null}<Button variant="contained" onClick={() => void rotateToken()}>Rotate registration token</Button></Stack></Page>;
}
...
function LegacyPage() {
    ...
    if (current.kind === "users" || current.kind === "tags" || current.kind === "categories") return <EntityListPage taxonomy={current.kind} />;
    if (current.kind === "user" || current.kind === "tag" || current.kind === "category") return <EntityPage taxonomy={current.kind === "user" ? "users" : current.kind === "tag" ? "tags" : "categories"} id={current.id!} />;
    if (current.kind === "user-posts" || current.kind === "tag-posts" || current.kind === "category-posts") return <EntityPostsPage taxonomy={current.kind === "user-posts" ? "users" : current.kind === "tag-posts" ? "tags" : "categories"} id={current.id!} />;
...
if (root) createRoot(root).render(<App><LegacyPage /></App>);
