...
function GitCommitsPage() {
    ...
    useEffect(() => {
        void fetch(GITHUB_COMMITS_URL, { headers: { Accept: "application/vnd.github+json" } })
            ...
            .catch((cause) =>
                setError(cause instanceof Error ? cause.message : "Unable to load git commits."),
            );
    }, []);
            ...
            <Stack spacing={2}>
                <Box>
                    ...
                    <Typography color="text.secondary">
                        Recent changes to the imshare source repository.
                    </Typography>
                </Box>
                {error ? (
                    <Stack spacing={1.5}>
                        ...
                        <Button
                            variant="outlined"
                            href="https://github.com/p0x38/imshare/commits/"
                            target="_blank"
                            rel="noopener noreferrer"
                        >
                            Open commit history on GitHub
                        </Button>
                    </Stack>
                ) : commits === null ? (
