...
function PostEditor() {
    ...
    async function submit(event: React.FormEvent) {
        ...
        try {
            if (editing) {
                const response = await api<{ data: Post }>(`/v1/posts/${encodeURIComponent(postId!)}`, { method: "PATCH", body: JSON.stringify({ title, caption: caption || null, description: description || null, sourceUrl: sourceUrl || null, originalCreator: originalCreator.trim() || null, originalCreatedAt: originalCreatedAtValue(), ...permalinkValues(), allowDownload, status, visibility, tags: tags.split(",").map((value) => value.trim()).filter(Boolean), categoryId: categoryId || null }) });
    ...
    return <Page maxWidth="md"><Stack spacing={2}><Typography variant="h4" component="h1">{editing ? "Edit Post" : "New Post"}</Typography>{error ? <Alert severity="error">{error}</Alert> : null}<Card variant="outlined"><CardContent><Stack component="form" spacing={2} onSubmit={submit}>{!editing ? <Box onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); addFiles(event.dataTransfer.files); }} sx={{ p: 2, border: 1, borderColor: dragging ? "primary.main" : "divider", borderRadius: 2, bgcolor: dragging ? "action.hover" : "transparent" }}><Button variant="outlined" component="label">Choose images<input hidden type="file" accept="image/*" multiple onChange={(event) => addFiles(event.target.files ?? [])} /></Button><Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>Drop images here or use the file picker.</Typography>{files.length ? <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 1.5, mt: 2 }}>{files.map((file, index) => <Card key={`${file.name}-${index}`} variant="outlined"><Box component="img" src={URL.createObjectURL(file)} alt={file.name} sx={{ width: "100%", aspectRatio: "1", objectFit: "cover" }} /><CardContent sx={{ p: 1 }}><Typography variant="body2" noWrap>{index + 1}. {file.name}</Typography><Stack direction="row" spacing={0.25} justifyContent="flex-end"><IconButton size="small" aria-label="Move image up" disabled={index === 0} onClick={() => moveFile(index, -1)}><ArrowUpward fontSize="small" /></IconButton><IconButton size="small" aria-label="Move image down" disabled={index === files.length - 1} onClick={() => moveFile(index, 1)}><ArrowDownward fontSize="small" /></IconButton><IconButton size="small" color="error" aria-label="Remove image" onClick={() => removeFile(index)}><Delete fontSize="small" /></IconButton></Stack></CardContent></Card>)}</Box> : null}</Box> : null}<TextField label="Title" value={title} onChange={(e) => setTitle(e.target.value)} required inputProps={{ maxLength: 200 }} />{!editing && files.length > 1 ? <FormControlLabel control={<Checkbox checked={mergeIntoMultiPost} onChange={(e) => setMergeIntoMultiPost(e.target.checked)} />} label="Merge all images into one multi-image post" /> : null}<Typography variant="body2" color="text.secondary">{!editing && files.length > 1 ? mergeIntoMultiPost ? "One draft post will contain all selected images." : "Each image will become its own draft post." : ""}</Typography><TextField label="Image caption" value={caption} onChange={(e) => setCaption(e.target.value)} multiline minRows={2} inputProps={{ maxLength: 10000 }} /><TextField label="Description" value={description} onChange={(e) => setDescription(e.target.value)} multiline minRows={4} /><TextField label="Source URL" type="url" value={sourceUrl} onChange={(e) => setSourceUrl(e.target.value)} /><Box><Typography variant="subtitle2" gutterBottom>Original creator</Typography><Stack spacing={2}><TextField label="Creator / author name" value={originalCreator} onChange={(e) => setOriginalCreator(e.target.value)} inputProps={{ maxLength: 500 }} helperText="Credit the original creator of the content, when known." /><TextField label="Original creation date" type="datetime-local" value={originalCreatedAt} onChange={(e) => setOriginalCreatedAt(e.target.value)} InputLabelProps={{ shrink: true }} helperText="When the original content was created, when known." /></Stack></Box><Box><Typography variant="subtitle2" gutterBottom>Public URL</Typography><Stack spacing={2}><FormControl fullWidth><InputLabel id="permalink-pattern-label">URL pattern</InputLabel><Select labelId="permalink-pattern-label" label="URL pattern" value={permalinkPattern} onChange={(e) => setPermalinkPattern(e.target.value as PermalinkPattern)}><MenuItem value="user">/user/id</MenuItem><MenuItem value="posts">/posts/id</MenuItem></Select></FormControl><FormControl fullWidth><InputLabel id="permalink-id-type-label">ID type</InputLabel><Select labelId="permalink-id-type-label" label="ID type" value={permalinkIdType} onChange={(e) => setPermalinkIdType(e.target.value as PermalinkIdType)}><MenuItem value="internalId">Internal ID</MenuItem><MenuItem value="normalizedTitle">Normalized title</MenuItem><MenuItem value="creationDate">Creation date</MenuItem><MenuItem value="custom">Custom ID</MenuItem></Select></FormControl>{permalinkIdType === "custom" ? <TextField label="Custom post ID" value={customPostId} onChange={(e) => setCustomPostId(e.target.value)} required inputProps={{ maxLength: 200 }} helperText="This value becomes the public identifier for the post." /> : null}</Stack></Box><FormControlLabel control={<Checkbox checked={allowDownload} onChange={(e) => setAllowDownload(e.target.checked)} />} label="Allow visitors to download the original image" />{editing ? <><FormControl fullWidth><InputLabel id="status-label">Status</InputLabel><Select labelId="status-label" label="Status" value={status} onChange={(e) => setStatus(e.target.value as PostStatus)}><MenuItem value="draft">Draft</MenuItem><MenuItem value="published">Published</MenuItem></Select></FormControl><FormControl fullWidth><InputLabel id="visibility-label">Visibility</InputLabel><Select labelId="visibility-label" label="Visibility" value={visibility} onChange={(e) => setVisibility(e.target.value as PostVisibility)}><MenuItem value="public">Public</MenuItem><MenuItem value="unlisted">Unlisted</MenuItem><MenuItem value="private">Private</MenuItem></Select></FormControl></> : null}{!editing ? <FormControl fullWidth><InputLabel id="visibility-new-label">Visibility</InputLabel><Select labelId="visibility-new-label" label="Visibility" value={visibility} onChange={(e) => setVisibility(e.target.value as PostVisibility)}><MenuItem value="public">Public</MenuItem><MenuItem value="unlisted">Unlisted</MenuItem><MenuItem value="private">Private</MenuItem></Select></FormControl> : null}<TextField label="Tags" value={tags} onChange={(e) => setTags(e.target.value)} helperText="Separate tags with commas" /><FormControl fullWidth><InputLabel id="category-label">Category</InputLabel><Select labelId="category-label" label="Category" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}><MenuItem value="">None</MenuItem>{categories.map((category) => <MenuItem key={category.id} value={category.id}>{category.name}</MenuItem>)}</Select></FormControl>{saving && !editing ? <Stack spacing={0.5}><LinearProgress variant={progress ? "determinate" : "indeterminate"} value={progress} /><Typography variant="body2" color="text.secondary">{uploadStatus}</Typography></Stack> : null}<Stack direction="row" spacing={1}><Button type="submit" variant="contained" disabled={saving}>{saving ? "Saving…" : editing ? "Save changes" : "Upload & create drafts"}</Button>{editing ? <Button type="button" color="error" variant="outlined" onClick={() => void deletePost()} disabled={saving}>Delete post</Button> : null}</Stack></Stack></CardContent></Card></Stack></Page>;
}
