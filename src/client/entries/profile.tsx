...
function ProfilePage() {
    ...
    const [profile, setProfile] = useState<Profile | null>(null); const [posts, setPosts] = useState<Post[]>([]); const [tab, setTab] = useState<ProfileTab>("posts"); const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
    ...
    const visibleTabs: Array<{ value: ProfileTab; label: string }> = [{ value: "posts", label: "Posts" }, { value: "about", label: "About" }, ...(profile.stats.followers !== null ? [{ value: "followers" as const, label: `Followers ${profile.stats.followers}` }] : []), ...(profile.stats.following !== null ? [{ value: "following" as const, label: `Following ${profile.stats.following}` }] : [])];
...
function pending(status: Profile["followStatus"]) { return status === "pending"; }
