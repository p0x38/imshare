        ...
        void api<{ data: Post }>(`/v1/posts/${encodeURIComponent(id)}`)
            ...
            .catch((cause) => {
                ...
                if (status === 401) window.location.href = "/account/login/";
                    ...
                    setError(
                        cause instanceof Error ? cause.message : t("dashboardPost.deleteError"),
                    );
            });
    }, [id, t]);
    async function remove() {
        if (!window.confirm(t("dashboardPost.deleteConfirm"))) return;
        setDeleting(true);
        setError("");
        try {
            ...
            setError(cause instanceof Error ? cause.message : t("dashboardPost.deleteError"));
            setDeleting(false);
        }
    }
    if (error && !post)
        ...
        <Page maxWidth="md">
            <Stack spacing={2}>
                ...
                <Typography color="text.secondary">
                    {t("dashboardPost.manageDescription")}
                </Typography>
