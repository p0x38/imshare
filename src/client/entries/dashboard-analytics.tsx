        ...
        <Stack spacing={3}>
            ...
            {data ? <>
                <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2, 1fr)", lg: "repeat(3, 1fr)" }, gap: 2 }}>
                    <Stat label="Posts" value={data.totals.posts} /><Stat label="All-time views" value={data.totals.views} /><Stat label="Followers" value={data.totals.followers} />
                ...
                <Card variant="outlined"><CardContent><Stack spacing={1.5}>
                    ...
                    {data.topPosts.length ? data.topPosts.map((post) => <Stack key={post.postId} direction="row" spacing={2}><Typography component="a" href={"/posts/" + encodeURIComponent(post.postId) + "/"} sx={{ flex: 1, color: "inherit" }}>{post.title}</Typography><Typography color="text.secondary">{post.views.toLocaleString()} views</Typography></Stack>) : <Typography color="text.secondary">No views recorded in this period.</Typography>}
                </Stack></CardContent></Card>
                <Alert severity="info">Analytics are limited to your own posts. Anonymous views are included in view totals, while unique-viewer counts only include signed-in viewers.</Alert>
            </> : !error ? <Typography color="text.secondary">Loading analytics…</Typography> : null}
        </Stack>
    </Page>;
}
const root = document.querySelector("#dashboard-analytics-page");
if (root) createRoot(root).render(<App><CreatorAnalytics /></App>);
