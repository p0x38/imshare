...
function AccountPage() {
    ...
    async function load() {
        try {
            ...
            setProfileLinks(
                (next.profileLinks || []).map(({ id, label, url }) => ({ id, label, url })),
            );
        } catch (cause) {
            if ((cause as Error & { status?: number }).status === 401) setUser(false);
            else setError(cause instanceof Error ? cause.message : t("accountPage.loadError"));
        }
    }
    useEffect(() => {
        void load();
    ...
    async function save(event: React.FormEvent) {
        ...
        try {
            await api(`/v1/users/${encodeURIComponent(user.id)}`, {
                ...
                body: JSON.stringify({
                    name,
                    handle: handle.trim().replace(/^@/, "").toLowerCase() || null,
                    bio: bio || null,
            ...
            for (const link of user.profileLinks || [])
                if (!currentIds.has(link.id))
                    await api(`/v1/me/links/${encodeURIComponent(link.id)}`, { method: "DELETE" });
            await load();
        } catch (cause) {
            setError(cause instanceof Error ? cause.message : t("accountPage.saveError"));
        } finally {
            setSaving(false);
        }
    }
    ...
    if (user === null)
        return (
            <Page>
                <LoadingState label={t("accountPage.loadingAccount")} />
            </Page>
        );
    if (user === false)
        return (
            <Page>
                <Card variant="outlined">
                    <CardContent>
                        ...
                        <Typography paragraph>{t("accountPage.notSignedIn")}</Typography>
                        <Button variant="contained" component="a" href="/account/login/">
                            {t("accountPage.logIn")}
                        </Button>
                        <Button component="a" href="/account/register/" sx={{ ml: 1 }}>
                            {t("accountPage.createAccount")}
                        </Button>
                    </CardContent>
                </Card>
            </Page>
            ...
            <Stack spacing={2}>
                ...
                <Card variant="outlined" component="form" onSubmit={save}>
                    <CardContent>
                        ...
                        <Stack spacing={2}>
                            ...
                            />
                            <TextField
                                label={t("accountPage.customHandle")}
                                value={handle}
                                onChange={(e) =>
                                    setHandle(e.target.value.replace(/^@/, "").toLowerCase())
                                }
                                placeholder="p0x38"
                                helperText={t("accountPage.handleHelp")}
                            />
                            <TextField
                                label={t("accountPage.profileDescription")}
                                value={bio}
                            ...
                            <FormControl fullWidth>
                                <InputLabel id="avatar-mode-label">
                                    {t("accountPage.profileAvatar")}
                                </InputLabel>
                                <Select
                                    labelId="avatar-mode-label"
                                    label={t("accountPage.profileAvatar")}
                                    value={avatarMode}
                                    onChange={(e) => setAvatarMode(e.target.value)}
                                >
                                    <MenuItem value="initials">
                                        {t("accountPage.textInitials")}
                                    </MenuItem>
                                    <MenuItem value="default">
                                        {t("accountPage.defaultAvatar")}
                                    </MenuItem>
                                    <MenuItem value="identicon">
                                        {t("accountPage.identicon")}
                                    </MenuItem>
                                    <MenuItem value="gravatar">
                                        {t("accountPage.gravatar")}
                                    </MenuItem>
                                    <MenuItem value="custom">{t("accountPage.custom")}</MenuItem>
                                </Select>
                            </FormControl>
                            <TextField
                            ...
                            />
                            <TextField
                                label={t("accountPage.website")}
                                type="url"
                                value={websiteUrl}
                                onChange={(e) => setWebsiteUrl(e.target.value)}
                            />
                            <TextField
                                label={t("accountPage.github")}
                                type="url"
                                value={githubUrl}
                            ...
                            <Stack spacing={1}>
                                <Typography variant="h6" component="h2">
                                    {t("accountPage.profileLinks")}
                                </Typography>
                                <Typography variant="body2" color="text.secondary">
                                    {t("accountPage.profileLinksHelp")}
                                </Typography>
                                {profileLinks.map((link, index) => (
                                    ...
                                    >
                                        ...
                                        />
                                        <IconButton
                                            aria-label={t("accountPage.deleteProfileLink")}
                                            onClick={() =>
                                                setProfileLinks((c) =>
                                ...
                                <Button
                                    startIcon={<Add />}
                                    onClick={() =>
                                        setProfileLinks((c) => [...c, { label: "", url: "" }])
                                    }
                                    disabled={profileLinks.length >= 20}
                                >
                                    {t("accountPage.addProfileLink")}
                                </Button>
                            </Stack>
                            <Button type="submit" variant="contained" disabled={saving}>
                                {saving ? t("accountPage.saving") : t("accountPage.saveProfile")}
                            </Button>
                        </Stack>
                    </CardContent>
                </Card>
                <Stack direction="row" spacing={1}>
                    <Button variant="outlined" component="a" href="/dashboard/">
                        {t("accountPage.dashboard")}
                    </Button>
                    <Button component="a" href="/account/logout/">
